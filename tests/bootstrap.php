...
/**
 * @file plugins/generic/assignEditorGeneral/tests/bootstrap.php
 *
 ...
 * plugins/generic/assignEditorGeneral, the OMP 3.5 installation around it is bootstrapped so the
...
if (!class_exists('\PKP\plugins\GenericPlugin')) {
    ...
    if (!is_file($root . '/lib/pkp/includes/bootstrap.php')) {
        fwrite(STDERR, "The plugin must be installed in plugins/generic/assignEditorGeneral of an OMP 3.5 installation to run the suite.\n");
        exit(2);
    }
    chdir($root);
...
require_once dirname(__DIR__) . '/AssignEditorGeneralPlugin.php';
